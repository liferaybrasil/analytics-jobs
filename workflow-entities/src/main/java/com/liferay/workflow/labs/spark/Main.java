...
public class Main {
	...
	protected static void doRun() {
		...
		SparkSession spark =
			SparkSession.builder().appName("Workflow Entities").config(
		...
		Dataset<Row> analyticsEventDataSet1 =
			spark.read().format("org.apache.spark.sql.cassandra").options(
				_analyticsEventOptions).load();

		analyticsEventDataSet1 = analyticsEventDataSet1.filter(
			"eventid = '" + _eventId1 + "' and createdate > '" + _last5Minutes +
				"'");

		analyticsEventDataSet1 = analyticsEventDataSet1.select(
			col("eventproperties").getField("kaleoDefinitionVersionId").as(
				"id"),
			col("eventproperties").getField("name").as("name"));

		analyticsEventDataSet1 = analyticsEventDataSet1.withColumn(
			"entity", lit("KALEO_DEFINITION_VERSION"));

		Dataset<Row> analyticsEventDataSet2 =
			spark.read().format("org.apache.spark.sql.cassandra").options(
				_analyticsEventOptions).load();

		analyticsEventDataSet2 = analyticsEventDataSet2.filter(
			"eventid = '" + _eventId2 + "' and createdate > '" + _last5Minutes +
				"'");
		...
		analyticsEventDataSet2 =
			analyticsEventDataSet2.withColumn("entity", lit("KALEO_TASK"));

		Dataset<Row> analyticsEventDataSet =
			analyticsEventDataSet1.union(analyticsEventDataSet2);

		analyticsEventDataSet.write().format(
